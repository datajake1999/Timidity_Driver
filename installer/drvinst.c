...
const char TIMIDITY_DRIVER_NAME[] = "timidity.dll";
const char TIMIDITY_CPLAPPLET_NAME[] = "timcfg.cpl";
const char TIMIDITY_SETUPTOOL_NAME[] = "timcfg.exe";
...
const char SUCCESSFULLY_INSTALLED_MSG[] = "Timidity MIDI Driver successfully installed";
const char SUCCESSFULLY_UPDATED_MSG[] = "Timidity MIDI Driver successfully updated";
const char SUCCESSFULLY_UNINSTALLED_MSG[] = "Timidity MIDI Driver successfully uninstalled";
...
const char CANNOT_INSTALL_NO_PORTS_ERR[] = "Cannot install Timidity MIDI driver:\n There is no MIDI ports available";
const char CANNOT_REGISTER_ERR[] = "Cannot register driver";
const char CANNOT_UNINSTALL_ERR[] = "Cannot uninstall Timidity MIDI driver";
const char CANNOT_UNINSTALL_NOT_FOUND_ERR[] = "Cannot uninstall Timidity MIDI driver:\n There is no driver registry entry found";
const char CANNOT_INSTALL_PATH_TOO_LONG_ERR[] = "Timidity MIDI Driver cannot be installed:\n Installation path is too long";
const char CANNOT_INSTALL_FILE_COPY_ERR[] = "Timidity MIDI Driver failed to install:\n File copying error";
...
BOOL registerDriver(BOOL *installMode)
{
	...
	for(i = 0; i < 10; i++)
	{
		...
		if(!_stricmp(str, TIMIDITY_DRIVER_NAME))
		{
			RegCloseKey(hReg);
			*installMode = FALSE;
			return TRUE;
	...
	res = RegSetValueExA(hReg, drvName, 0, REG_SZ, (LPBYTE)TIMIDITY_DRIVER_NAME, sizeof(TIMIDITY_DRIVER_NAME));
	if(res != ERROR_SUCCESS)
	{
...
int main(int argc, char *argv[])
{
	...
	{
		constructDriverPathName(pathName, TIMIDITY_DRIVER_NAME);
		deleteFileReliably(pathName, TIMIDITY_DRIVER_NAME);
		constructDriverPathName(pathName, TIMIDITY_CPLAPPLET_NAME);
		deleteFileReliably(pathName, TIMIDITY_CPLAPPLET_NAME);
		constructDriverPathName(pathName, TIMIDITY_SETUPTOOL_NAME);
		deleteFileReliably(pathName, TIMIDITY_SETUPTOOL_NAME);
	...
	if(!copyFileIntoSystem(TIMIDITY_DRIVER_NAME, argv))
		return 4;
	if(!copyFileIntoSystem(TIMIDITY_CPLAPPLET_NAME, argv))
		return 4;
	if(!copyFileIntoSystem(TIMIDITY_SETUPTOOL_NAME, argv))
		return 4;
